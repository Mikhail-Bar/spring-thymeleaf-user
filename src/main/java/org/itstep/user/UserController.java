...
public class UserController {
    ...
    @PostMapping("/signup")
    public String signupPost(User user, Model model, @RequestParam(value = "password2" , required = true) String password2, HttpServletResponse response) {
        //Передать id в заголовке ответа
        //System.out.println(user.getPassword());
        //System.out.println(password2);
        if (userService.getUserByUsername(user.getUsername())!=null)
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "User with this username or email already exists");
        if (!user.getPassword().equals(password2))
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Passwords do not match");
